...
// Função para pegar a data de hoje formatada corretamente (YYYY-MM-DD) no fuso local
...
export function Home() {
  ...
  // Estado para armazenar a data selecionada no Dashboard

  const [filtroData, setFiltroData] = useState(getHojeStr());
  ...
  // 3. BUSCA DE DADOS COM O PARÂMETRO DA DATA
  ...
  useEffect(() => {
    ...
    const fetchAnalytics = async () => {
      try {
        ...
        // Montamos o parâmetro de URL da data que será enviado ao Django

        let queryParams = `data=${filtroData}`;
        ...
        if (user?.cargo === "VENDEDOR") {
          ...
          if (idLojaSupervisor) {
            queryParams += `&loja_id=${idLojaSupervisor}`;
          }
    ...
    fetchAnalytics();
  }, [user, user?.cargo, filtroData]); // Recarrega sempre que a data mudar
  ...
  // FILTRAGEM (Pesquisa de texto e garantia da data)

  const tabelaFiltrada = (data?.tabela || []).filter((item) => {
    let dataItemStr = "";

    // Converte a data UTC do servidor para a data real do fuso horário local

    if (item.data_hora) {
      const dataVenda = new Date(item.data_hora);

      const tzoffset = dataVenda.getTimezoneOffset() * 60000;

      dataItemStr = new Date(dataVenda.getTime() - tzoffset)
    ...
    const dataConfere = dataItemStr === filtroData;
    ...
    return dataConfere && textoConfere;
  });
  ...
  return (
    <div className="max-w-7xl mx-auto flex flex-col gap-8 animate-in fade-in duration-500 relative">
      {/* Overlay translúcido sobre o painel enquanto carrega uma nova data */}
      ...
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4 bg-white p-6 rounded-3xl shadow-lg border border-blue-50">
      ...
      <div className="bg-white border border-blue-50 rounded-[2rem] shadow-xl overflow-hidden">
        {/* Nova barra superior da tabela: Pesquisa + Data + Contador */}
        ...
        <div className="p-6 bg-slate-50/50 border-b border-slate-100 flex flex-col lg:flex-row justify-between items-start lg:items-center gap-4">
          ...
          <div className="flex flex-col sm:flex-row items-start sm:items-center gap-4 w-full lg:w-auto justify-between">
            {/* CAMPO DE SELEÇÃO DE DATA AQUI */}

            <div className="flex items-center bg-white border border-slate-200 rounded-xl px-4 py-2.5 hover:border-[#4D7BAB] transition-colors focus-within:border-[#4D7BAB] focus-within:ring-2 focus-within:ring-[#4D7BAB]/20 w-full sm:w-auto">
              ...
              <input
                ...
                className="bg-transparent border-none outline-none text-slate-700 font-bold cursor-pointer w-full"
              />
            </div>

            <div className="text-sm font-bold text-slate-400 whitespace-nowrap">
              {tabelaFiltrada.length} registro(s) encontrado(s)
            </div>
          </div>
        </div>

        {/* Tabela com scroll vertical interno */}
            ...
            <tbody className="divide-y divide-slate-100">
              {tabelaFiltrada.length === 0 ? (
                <tr>
                  ...
                  >
                    Nenhum registro encontrado para esta data ou filtro.
                  </td>
                </tr>
              ) : (
